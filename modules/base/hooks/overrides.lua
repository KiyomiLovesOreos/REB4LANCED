-- Square-card sprite fix: display_size sets T correctly but SMODS's set_sprites only
-- applies scale.y = scale.x for "Square Joker" by name.  We replicate that logic for
-- any center flagged with square_card = true.
local reb4l_set_sprites = Card.set_sprites
function Card:set_sprites(center, ...)
    reb4l_set_sprites(self, center, ...)
    local c = self.config and self.config.center
    if c and c.square_card and (c.discovered or self.bypass_discovery_center)
        and self.children.center then
...
function Blind:debuff_card(card, from_blind)
    ...
    if self.name == 'Crimson Heart' and not self.disabled and card.area == G.jokers then
        card:set_debuff(card.ability.crimson_heart_chosen and true or false)
        return
    end
...
-- Intercepts every get_id() call so the two selected ranks swap with each other
    ...
    if id and G.jokers then
        for _, j in ipairs(G.jokers.cards) do
            if j.config.center.key == 'j_reb4l_court_jester'
                ...
                and j.ability.extra.target_rank > 0 then
                if id == j.ability.extra.source_rank then
                    return j.ability.extra.target_rank
                elseif id == j.ability.extra.target_rank then
                    return j.ability.extra.source_rank
                end
            end
        end
    end
...
function Game:update(dt)
    ...
    -- Wild Card suit-based debuff resistance
...
-- Money gain hook for Black Card
local reb4l_orig_ease_dollars = ease_dollars
local reb4l_easing_dollars = false
function ease_dollars(mod, instant)
    if not reb4l_easing_dollars and mod and type(mod) == 'number' and mod > 0 then
        reb4l_easing_dollars = true
        SMODS.calculate_context({ reb4l_money_gain = true, amount = mod })
        reb4l_easing_dollars = false
    end
    reb4l_orig_ease_dollars(mod, instant)
end

-- first_hand_drawn hook for Palette
local reb4l_orig_draw_from_deck = G.FUNCS.draw_from_deck_to_hand
G.FUNCS.draw_from_deck_to_hand = function(e)
    local result = reb4l_orig_draw_from_deck(e)
    if G.GAME and G.GAME.current_round and G.GAME.current_round.hands_played == 0
       and G.GAME.current_round.discards_used == 0
       and not G.GAME.current_round.reb4l_first_hand_drawn_triggered then
        G.GAME.current_round.reb4l_first_hand_drawn_triggered = true
        G.E_MANAGER:add_event(Event({
            trigger = 'after',
            delay = 0.4,
            func = function()
                SMODS.calculate_context({
                    ...
                    hand_drawn = G.hand.cards -- Pass the cards actually in hand
                })
                return true
            end
        }))
...
-- Probability display fix for Oops! No Sixes
local reb4l_old_get_prob_vars = SMODS.get_probability_vars
function SMODS.get_probability_vars(trigger_obj, base_numerator, base_denominator, identifier, from_roll, no_mod)
    local num, den = reb4l_old_get_prob_vars(trigger_obj, base_numerator, base_denominator, identifier, from_roll, no_mod)
        ...
        if area.cards then
            for _, c in ipairs(area.cards) do
                if not c.debuff then
                    local target = nil
                    if c.config.center.key == 'j_blueprint' or c.config.center.key == 'j_brainstorm' then
                        target = c.ability.blueprint_target or c.ability.brainstorm_target
                    end
                    if c.config.center.key == 'j_reb4l_oops_no_sixes' or (target and target.config.center.key == 'j_reb4l_oops_no_sixes') then
                        no_sixes = no_sixes + 1
                    end
                end
            end
