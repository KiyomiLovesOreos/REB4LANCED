...
local defaults = {
    ...
    palette_enhanced = false,
    oops_no_sixes_enhanced = false,
    black_card_enhanced = false,
    ...
    -- New tag sets
    tag_set_skip = false,
    magnet_vouchers = false,
    -- New boss blinds
    nc_boss_blinds = false,
    -- New consumable sets
    axiom_set = false,
    spectral_set_sell_value = false,
...
if type(REB4LANCED.config.perishable_enhanced) == 'boolean' then
    REB4LANCED.config.perishable_enhanced = REB4LANCED.config.perishable_enhanced and 2 or 1
end
...
REB4LANCED.config.perishable_enhanced = math.max(1, math.min(3, tonumber(REB4LANCED.config.perishable_enhanced) or 1))
REB4LANCED.config.purple_stake_mode   = math.max(1, math.min(2, tonumber(REB4LANCED.config.purple_stake_mode)   or 1))
...
-- ─── Preset keys / suggested values → see presets.lua ────────────────────────
...
-- A set box is a single toggle that enables/disables all jokers in a themed group.
-- `members` is a list of short strings shown as a subtitle beneath the label.
local function make_set_box(label, key, members)
    local toggle = create_toggle({
        label = label,
        ref_table = REB4LANCED.config,
        ref_value = key,
    ...
    for _, m in ipairs(members) do
        col_nodes[#col_nodes + 1] = {
            ...
            nodes = { { n = G.UIT.T, config = { text = m, scale = 0.28, colour = HEX('b8c7d4') } } }
        }
    end
...
local function get_category_options(key)
    ...
    elseif key == 'misc' then
        return {
            ...
            make_cycle_box('Perishable', 'perishable_enhanced', { 'Vanilla', 'Wilting', 'Sell Decay' }, 8.5),
            make_cycle_box('Purple Stake', 'purple_stake_mode', { 'Voucher Cost', 'Pinned' }, 8.5),
            make_option_box('Interest on Skip', 'Earn interest when skipping a blind', 'interest_on_skip'),
            make_option_box('Black Card', '+$1 when gaining money', 'black_card_enhanced'),
    ...
    elseif key == 'nc_jokers' then
        -- Grouped by theme/set; toggling a set enables all items within it.
        return {
            make_set_box('Rank-Specific', 'joker_set_rank', {
                'Enables all rank-specific jokers from the mod.'
            }),
            make_set_box('Stone-Specific', 'joker_set_stone', {
                'Enables all stone-specific content',
            }),
            make_set_box('Suit/Wild Specific', 'joker_set_suit', {
                'Enables all suit- and wild-specific jokers (including Palette).',
            }),
            make_set_box('Boss Blinds', 'nc_boss_blinds', {
                'The Raging Pyre: blind grows X1.1 per scoring trigger',
            }),
            make_set_box('Probability Manipulation', 'oops_no_sixes_enhanced', {
                'Oops! No Sixes: doubles probability denominators',
            }),
            make_set_box('Vouchers', 'magnet_vouchers', {
                'Magnet: see top 2 cards of your deck while playing',
                'Electromagnet: see 2 more on top of that',
            }),
            make_set_box('Skip Tags', 'tag_set_skip', {
                'Grasp Tag: +1 hand size for the run',
                'Surplus Tag: +1 consumable slot',
            }),
            make_set_box('Axioms', 'axiom_set', {
                'Enables the Axiom consumable set.',
            }),
            make_set_box('Sell Value', 'spectral_set_sell_value', {
                'Shatter: destroy a Joker; all cards gain XMult when scored',
            }),
            make_set_box('Debt', 'spectral_set_debt', {
                '(Placeholder — not yet implemented)',
            }),
        }
    elseif key == 'nc_decks' then
        return {
            make_option_box('Workshop Deck', 'Start with Overstock and Clearance Sale', 'workshop_deck'),
            make_option_box('Magician Deck', 'Start with Magic Trick and Illusion vouchers', 'magician_deck'),
            make_option_box('Draft Deck', 'Draft 3 decks from 3 sets of 3; gain all benefits and drawbacks', 'draft_deck'),
            make_option_box('Aperture Deck', 'See the top 5 cards of your deck at all times; -1 hand per round', 'all_seeing_eye_deck'),
    ...
    elseif key == 'presets' then
        local code = REB4LANCED.UI.encode_config()
...
-- Exposed via REB4LANCED.UI so presets.lua callbacks can call it.
...
REB4LANCED.UI.refresh_panel = reb4l_refresh_panel
