...
-- Give Pinned a badge sprite (vanilla has none) and fix the draw path.
-- Done via direct object mutation rather than take_ownership to avoid mod attribution badges.
for _, s in ipairs(SMODS.Sticker.obj_buffer) do
    if s.key == 'pinned' then
        s.atlas = 'reb4l_stickers'
        s.pos   = { x = 0, y = 0 }
        local _orig_apply = s.apply
        s.apply = function(self, card, val)
            _orig_apply(self, card, val)
